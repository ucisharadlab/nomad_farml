...
import { ArrowUpTrayIcon, TrashIcon, PlusIcon, PlayIcon, InformationCircleIcon, SparklesIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
...
const ModelManagement = ({ models, setModels, setStatus }) => {
    ...
    const handleAddModel = async () => {
        setIsLoading(true);
        setStatus({ message: null, type: 'info' });
        ...
        if (newModel.type === 'custom') {
            ...
            try {
                ...
                modelPayload = {
                  ...modelPayload,
                  is_custom: true,
                  type: uploadData.class_name,
                  module_name: uploadData.module_name,
        ...
        try {
            const addRes = await fetch(`${API_BASE_URL}/api/models`, {
                method: 'POST',
            ...
            fetchModels(); // Refresh list from backend
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                        {MODEL_PARAM_TEMPLATES[newModel.type].map(param => (
                          ...
                          <Select 
                            ...
                            value={newModel.params[param.name] || ''}
                          >
...
const DataUpload = ({ setStatus, setInitialResults, setNomadConfigReady }) => {
    ...
    const handleUpload = async () => {
        if (!file) {
            setStatus({ message: 'Please select a CSV file to upload.', type: 'error' });
            return;
        }
        setIsLoading(true);
        setStatus({ message: 'Uploading and training models... This may take a moment.', type: 'info' });
        ...
        try {
            ...
            setStatus({ message: `Training complete. Detected classes: ${data.classes_str.join(', ')}`, type: 'success' });
    ...
    return (
        <Section title="Upload Data & Train Base Models" step="2">
            <p className="text-gray-600 mb-4">Upload a CSV where the second to last column is the target variable.</p>
            <div className="flex flex-col sm:flex-row items-center gap-4">
                <Input type="file" accept=".csv" onChange={(e) => setFile(e.target.files[0])} className="flex-grow"/>
                <Button onClick={handleUpload} disabled={isLoading} icon={ArrowUpTrayIcon}>
...
const InitialTrainingSummary = ({ initialResults }) => (
    <div className="mt-6">
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Individual Model Performance (on Test Set)</h3>
        <div className="overflow-x-auto bg-white rounded-lg shadow">
            <table className="w-full text-sm text-left text-gray-500">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" className="px-6 py-3">Weighted F1</th>
                        <th scope="col" className="px-6 py-3">Cost</th>
                    </tr>
                </thead>
                <tbody>
                    {initialResults.model_summaries.map(model => (
                        <tr key={model.name} className="bg-white border-b hover:bg-gray-50">
                            <th scope="row" className={`px-6 py-4 font-medium whitespace-nowrap ${model.name.includes('FAILED') ? 'text-red-600' : 'text-gray-900'}`}>{model.name}</th>
                            <td className="px-6 py-4">{model.accuracy.toFixed(4)}</td>
                            <td className="px-6 py-4">{model.f1_score_weighted.toFixed(4)}</td>
                            <td className="px-6 py-4">{model.cost}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
const NomadConfig = ({ initialResults, setSimulationConfig, setStatus }) => {
    const [config, setConfig] = useState({
        ...
        safety_check_type: 'conservative',
        adaptive_update_window: 100,
        adaptive_beta: 0.3,
    });
    ...
    const handleRunSimulation = () => {
        ...
        for (const phase of phases) {
          ...
          if (Math.abs(sum - 1.0) > 0.01) {
            setStatus({message: `Probabilities in a phase must sum to 1.0. Current sum is ${sum.toFixed(2)}`, type: 'error'});
            return;
          }
        }
    ...
    const addPhase = () => {
        setPhases(p => [...p, { duration: 100, target_distribution: initialResults.initial_priors_str_keys || {} }]);
    };
    
    const removePhase = (index) => {
        setPhases(p => p.filter((_, i) => i !== index));
    };

    const updatePhase = (index, field, value) => {
        setPhases(p => p.map((phase, i) => i === index ? { ...phase, [field]: value } : phase));
    };
    
    const handleResetPhaseDistribution = (phaseIndex) => {
        updatePhase(phaseIndex, 'target_distribution', initialResults.initial_priors_str_keys);
    };
    ...
    return (
        <Section title="Configure NOMAD Engine" step="3">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div>
                <label className="font-semibold text-gray-700 block mb-1">Adaptive Window</label>
                <Input type="number" name="adaptive_update_window" value={config.adaptive_update_window} onChange={handleInputChange} min="0" />
              </div>
               <div>
                <label className="font-semibold text-gray-700 block mb-1">Adaptive Beta (β)</label>
                <Input type="number" name="adaptive_beta" value={config.adaptive_beta} onChange={handleInputChange} step="0.05" min="0" max="1" />
              </div>
            </div>

            <div className="mt-8 pt-6 border-t">
                <h3 className="text-lg font-semibold text-gray-700 mb-2">Define Workload Phases (Optional)</h3>
                <p className="text-sm text-gray-500 mb-4">If empty, simulation runs on the test set split. Probabilities must sum to 1.0 per phase.</p>
                    ...
                    <div key={i} className="p-4 bg-gray-50 rounded-lg border">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => removePhase(i)} className="text-red-500 hover:text-red-700"><TrashIcon className="h-5 w-5"/></button>
                        </div>
                      </div>
                      ...
                      <div className="mt-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                        {Object.keys(phase.target_distribution).map(cn => (
                          <div key={cn}>
                            ...
                            <Input type="number" value={phase.target_distribution[cn]} onChange={e => updatePhaseDist(i, cn, e.target.value)} step="0.01" min="0" max="1"/>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="mt-8 text-center">
                <Button onClick={handleRunSimulation} icon={PlayIcon} className="text-lg px-8 py-3 bg-green-600 hover:bg-green-700">
...
const FinalSummary = ({ summaryData, classNames }) => {
    ...
    const ConfusionMatrix = ({ title, cmData }) => {
      if (!cmData || cmData.length === 0 || !classNames || classNames.length === 0) {
        return <p>Confusion Matrix not available.</p>;
      }
      return (
        <div className="mt-4">
...
export default function App() {
    ...
    const [status, setStatus] = useState({ message: null, type: 'info' });
    ...
    const chartDataRef = useRef({}); // Using ref to hold latest chart data for event listener
    ...
    useEffect(() => {
        ...
        es.onmessage = (event) => {
            ...
            if (data.type === 'setup') {
                ...
                // Initialize chart data structure in the ref
                ...
                setSimulationData(chartDataRef.current); // Initial render with empty structure
                setStatus({ message: `Simulation started. Total events: ${data.total_events}`, type: 'info' });

            } else if (data.type === 'batch_update') {
                // Update data in the ref
                ...
                // Trigger re-render by setting a new object to state
                ...
                {simulationData && simulationData.modelRunCounts.labels.length > 0 && (
                    <Section title="Live Simulation Results" step="4">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <ChartComponent type="line" chartData={simulationData.cumulativeCost} chartOptions={{ responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Cumulative Cost: NOMAD vs. Role Model' }}, scales: {y: {beginAtZero: true}} }} />
